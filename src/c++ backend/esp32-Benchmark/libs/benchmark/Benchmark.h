...
public:
    ...
    */
	 ...
	 *  @return	as a return value, we will expect the needed total execution time in ms needed for the whole task
	*/
	float performComputationBenchmark(int numOfThreads);
	...
	*/
	...
	/** start the Mandelbrot benchmark and set the number of threads, which will be created to solve the problem
	 *  with the help of parallel processing
	 *
	 *	@param	specify the filename in which the compressed fractal should be saved in (*.ppm)
	 *	@param	set width and height of main Mandelbrot image (max width, max height)
	 *  @param	specify the number @numOfThreads of threads; in general, this number can not be higher than the
	 *  		total number of dedicated cores on the ESP32 (cores: 2x) -> setMaxNumOfThreads()
	 *  @return	as a return value, we will expect the needed total execution time in ms needed for the whole task
	*/
	float performMandelbrotBenchmark(String ppmFilename, unsigned int width, unsigned int height, int numOfThreads);
...
private:
	...
	/** will be called form the performBenchmarkComputation() function, specify the number of created threads and the
	 *
	 *  @param	specify the number @numOfThreads of threads; in general, this number can not be higher than the
	 *  		total number of dedicated cores on the ESP32 (cores: 2x) -> setMaxNumOfThreads()
	...
	*/
	 ...
	 *  @return will return the needed execution time
	*/
	float runMandelbrotWith(String filename, unsigned int width, unsigned int height, int numOfThreads, int numOfCycles);
	 ...
	 *  coolDownTimer	delay in ms between different computations / perform...() calls
	 */
